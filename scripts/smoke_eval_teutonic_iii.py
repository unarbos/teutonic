#!/usr/bin/env python3
"""Smoke test eval of Teutonic-III without going through the prod eval server.

Runs eval_torch's MultiGPUEvaluator + run_bootstrap_test directly with king
== challenger == unconst/Teutonic-III. Validates: model loads at ~3.3B,
...
Usage on Targon (the GPU box):
    bash -c '. ~/env.sh && /root/eval-venv/bin/python smoke_eval_teutonic_iii.py'
"""
import argparse
import json
import logging
...
def main():
    ...
    ap.add_argument("--repo", default="unconst/Teutonic-III")
