#!/usr/bin/env python3
"""Phase B cutover script: archive old dashboard + wipe live Teutonic-I state.
...
What it does (in order):
  1. Read old `dashboard.json` from Hippius (and R2) -> save to
     `dashboard-teutonic-i-final.json` in both buckets (kept for posterity)
...
Usage:
    ...
    python scripts/cutover_to_teutonic_iii.py [--dry-run]
"""
import argparse
import json
import logging
...
def archive_dashboard(hippius, hippius_bucket, r2, r2_bucket, dry_run):
    archive_key = "dashboard-teutonic-i-final.json"
...
def delete_eval_objects(r2, r2_bucket, dry_run):
    log.info("deleting eval/* objects")
...
def main():
    ...
    log.info("=== Phase B cutover (dry_run=%s) ===", args.dry_run)
