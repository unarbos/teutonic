#!/usr/bin/env python3
"""Smoke test eval of Teutonic-II without going through the prod eval server.

Runs eval_torch's MultiGPUEvaluator + run_bootstrap_test directly with king
== challenger == unconst/Teutonic-II. Validates: model loads at 4B, probe
passes, paired loss == 0 (same model), no OOM, throughput estimate.
...
Usage on Targon (the GPU box):
    bash -c '. ~/env.sh && /root/eval-venv/bin/python smoke_eval_teutonic_ii.py'
"""
import argparse
import json
import logging
...
def main():
    ...
    ap.add_argument("--repo", default="unconst/Teutonic-II")
    ap.add_argument("--eval-n", type=int, default=128)
    ap.add_argument("--batch-size", type=int,
                    default=int(os.environ.get("EVAL_BATCH_SIZE", "256")))
